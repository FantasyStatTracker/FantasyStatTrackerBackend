...
@FullData.before_app_first_request
def run():
...
@FullData.route("/full-team-data-v2", methods=["GET"])
@cross_origin()
def get_team_data():
    res = {}
    for team in team_data_global:
        team_data = team_data_global[team]  
        res[team_data.get_name()] = team_data.team_stats.__dict__
    return jsonify(res)

@FullData.route("/winning-matchups-v2", methods=["GET"])
@cross_origin()
...
@FullData.route("/category-leader-v2", methods=["GET"])
@cross_origin()
